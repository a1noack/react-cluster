...
from distribute_experiments import check_joblib_dict
...
# def filter_df_with_pks(df, set_of_pks):
#     """
#     Use a set of PK to filter a dataframe.

#     You can get set_of_pks by the .keys() method of output of build_known_instances
#     """
#     assert(no_duplicate_index(df))
#     index_to_drop = [idx for idx in df.index if tuple(get_pk_tuple(df, idx)) not in set_of_pks]
...
def main(args, out_dir):
    ...
    print('\n--- creating joblib')
    csv_name = args.dataset + '.csv'
    print('\n' + csv_name)
    df = pd.read_csv(os.path.join(out_dir, csv_name))
    data = df_to_instance_subset(df, known_instances)
    joblib_name = args.dataset + '.joblib'
    joblib.dump(data, os.path.join(out_dir, joblib_name))
...
if __name__ == '__main__':
    ...
    csv_name = args.dataset + '.csv'
    assert os.path.exists(os.path.join(out_dir, csv_name)), 'cannot find csv in ' + out_dir
